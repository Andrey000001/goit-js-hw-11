  ...
  await fetchRequest(request)
    .then(data => {
        ...
        if (request === '') {
            ...
            Notify.warning('Вы ввели не коректно')
          }
          else {
...
export async function onLoadMoreData(e) {
    ...
    await fetchRequest(requestLoadMore ,page += 1)
    .then(data => {
        const totalHits = data.data.totalHits
        const delenie = Math.floor(data.data.totalHits / maxPerPage)
        if(page === delenie) {
            Notify.info('Больше фото нет')
            hideMoreBtn()
        }
        Notify.success(`Всего фото ${totalHits}`)
        renderingMarkup(data.data.hits)
    })
}
